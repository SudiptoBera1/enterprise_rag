...
from api.permissions import filter_authorized_doc_ids, require_role
from api.schemas import QueryRequest, QueryResponse
from api.service import rag_service
from billing.token_logger import token_logger
from evaluation.retrieval_metrics import evaluate_retriever, load_sample_dataset
...
async def rag_exception_handler(request: Request, exc: RAGException):
    logger.error(
        "rag_exception",
        extra={"error_code": exc.error_code, "error_message": exc.message, "error_details": exc.details},
    )
    return JSONResponse(
...
async def ask_question(request: Request, payload: QueryRequest, user: dict = Depends(get_current_user)):
    require_role(user, ["admin", "user"])
    METRICS["requests_total"] += 1
    start_time = time.time()
    api_key_id = user.get("api_key_id", "unknown")
    ...
    logger.info(
        "query_received",
        extra={"query": payload.query, "client_ip": request.client.host if request.client else None, "api_key_id": api_key_id, "role": user.get("role")},
    )

    try:
        await rag_service.ensure_initialized_async()
        all_doc_ids = [doc["doc_id"] for doc in rag_service.documents]
        allowed_doc_ids = filter_authorized_doc_ids(user, all_doc_ids)

        result = await rag_service.ask_async(payload.query, allowed_doc_ids=allowed_doc_ids)
        ...
        if telemetry.get("retrieval_ms") is not None:
            METRICS["retrieval_ms"].append(telemetry["retrieval_ms"])
        if telemetry.get("generation_ms") is not None:
            METRICS["generation_ms"].append(telemetry["generation_ms"])

        if getattr(rag_service, "vector_store", None):
            METRICS["cache_hits"] = rag_service.vector_store.cache_hits
            METRICS["cache_misses"] = rag_service.vector_store.cache_misses
        ...
        logger.info(
            ...
            extra={
                ...
                "generation_ms": telemetry.get("generation_ms", 0),
        ...
        logger.error(
            "unexpected_error",
            extra={"query": payload.query, "error_details": str(e), "error_type": type(e).__name__, "api_key_id": api_key_id},
        )
        raise ProcessingError(details="An unexpected error occurred. Please try again later.")


@app.get("/metrics", dependencies=[Depends(get_current_user)], tags=["Monitoring"])
async def metrics():
    total_cache = METRICS["cache_hits"] + METRICS["cache_misses"]
    cache_hit_ratio = round(METRICS["cache_hits"] / total_cache, 4) if total_cache else 0.0
    ...
    return {
        ...
        "retrieval_time_percentiles_ms": {
            "p50": _percentile(METRICS["retrieval_ms"], 50),
            "p95": _percentile(METRICS["retrieval_ms"], 95),
            "p99": _percentile(METRICS["retrieval_ms"], 99),
        },
        "token_usage": token_summary,
        "cost_estimates": {"estimated_total_cost_usd": token_summary["estimated_total_cost_usd"]},
    }


@app.get("/metrics/retrieval", dependencies=[Depends(get_current_user)], tags=["Monitoring"])
async def metrics_retrieval():
    await rag_service.ensure_initialized_async()
    dataset = load_sample_dataset()
    evaluation = evaluate_retriever(rag_service.hybrid, dataset, k=3)
    evaluation["latency_percentiles_ms"] = {
        "p50": _percentile(METRICS["retrieval_ms"], 50),
        "p95": _percentile(METRICS["retrieval_ms"], 95),
        "p99": _percentile(METRICS["retrieval_ms"], 99),
    }
...
@app.get("/metrics/token_usage", dependencies=[Depends(get_current_user)], tags=["Monitoring"])
async def metrics_token_usage():
    summary = token_logger.summary()
    return {
        "token_usage": summary,
        "cost_estimates": {"estimated_total_cost_usd": summary["estimated_total_cost_usd"]},
    }
